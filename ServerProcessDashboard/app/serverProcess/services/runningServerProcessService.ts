    ...
    export class RunningServerProcessService implements IRunningServerProcessService {
        ...
        public getById = (id: string) => {
            ...
            this.$http({ method: "GET", url: this.getBaseUri() + "getbyid?id=" + id }).then((results) => {
                this.dataStore.getByGuid = results.data;
                deferred.resolve(results.data);
            }).catch((error) => {
                deferred.reject(error);
            });
        ...
        public getByGuid = (guid: string) => {
            ...
            if (this.dataStore.getByGuid && this.dataStore.getByGuid.guid == guid) {
                deferred.resolve(this.dataStore.getByGuid);
                return deferred.promise;
            }

            this.$http({ method: "GET", url: this.getBaseUri() + "getByGuid?guid=" + guid }).then((results) => {
                this.dataStore.getByGuid = results.data;
                deferred.resolve(results.data);
            }).catch((error) => {
                deferred.reject(error);
